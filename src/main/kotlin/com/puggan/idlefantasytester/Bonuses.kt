...
class Bonuses(private val loadout: Loadout, private val skill: String) {
     ...
     */
    val capeMultiplier: Double =
        run {
            val bonus = GameData.capeBonusTotal(loadout.capes, skill)
            if (bonus <= 0.0) return@run 1.0
            val scaling =
                PrestigeBoosts.effectTotal(trees, flags, skill, PrestigeBoosts.CAPE_SCALING)
    ...
    fun bestTool(
        ...
        if (loadout.tools.isEmpty()) {
            ...
            GameData.bestAgilityTool(loadout.tools, level, activityLevelRequired)
        }
    ...
    fun blessingActive(elapsedMs: Long): Boolean =
        when {
